...
By the third epoch, the training loss further decreased to 0.0168, and the F1 score climbed to an impressive 0.9607. The validation loss also improved slightly to 0.0303. However, the loss curve shows early signs of divergence: the training loss continues to decrease substantially while the validation loss flattens. Although the validation loss is not yet increasing, this widening gap indicates the beginning of potential overfitting, as the model becomes more specialized to the training data without equivalent gains on unseen data.
